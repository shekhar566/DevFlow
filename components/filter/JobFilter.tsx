  ...
  return (
    <div className="relative mt-11 flex w-full justify-between gap-5 max-sm:flex-col sm:items-center">
      <LocalSearch
        ...
        iconPosition="left"
        imgSrc="/icons/job-search.svg"
        // PIVOT: Medical context placeholder
      ...
      <Select onValueChange={(value) => handleUpdateParams(value)}>
        ...
        {/* Added background class to fix dark mode dropdown visibility */}
            ...
            {Array.isArray(countriesList) && countriesList.length > 0 ? (
              countriesList.map((country: Country) => (
                <SelectItem
                  key={country.name.common}
                  value={country.name.common}
                  className="px-4 py-3 cursor-pointer focus:bg-light-800 dark:focus:bg-dark-400"
                >
                  {country.name.common}
                </SelectItem>
              ))
